...
def Quecpython_produce(file_data):
    # 生成工具
    name = "Quecpython.py"
    with open(name, "wb+") as f:
        f.write(file_data)
        f.close()
Quecpython_produce(Quecpython_file_data)
...
class uart_read_write(object):
    ...
    def module_api(self):
        cmd = f'python Quecpython.py -d {self.usb_port} -b {self.baudrate} ./uart_api_test.py'
        self.dos.commands(cmd,'',10)
